...
const download_extension = document.getElementById('download-extension')
const install_video = document.getElementById('install-video')
let currentIndex = 3;
let isAnimating = false;


download_extension.addEventListener('click', download_extension_zip);
function download_extension_zip() {
    const link = document.createElement('a');
    link.href = './Desoft.zip'; // путь к файлу ZIP — тот, что лежит рядом со скриптом
    link.download = 'Desoft.zip'; // имя файла при скачивании
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
}

install_video.addEventListener('click', open_video);
function open_video() {
    const link = document.createElement('a');
    link.href = './Desoft.zip'; // путь к файлу ZIP — тот, что лежит рядом со скриптом
    document.body.appendChild(link);
...
const imageData = [
    {
        title: "პირადი ბაზა",
        description: "სისტემა ინახავს თქვენს განცხადებებს"
    },
    {
        title: "დახვეწილი ინტერფეისი",
        description: "სისტემა არის შექმნილი ბროკერებისთვის"
    },
    {
        title: "სუფთა ფოტოები",
        description: "შესაძლოა ფოტოების წმენა ლოგოებისგან"
    },
    {
        title: "უძრავი ქონება",
        description: "ავტომატიზაცია / ფორმების შევსება"
    },
    {
        title: "Desert Dunes",
